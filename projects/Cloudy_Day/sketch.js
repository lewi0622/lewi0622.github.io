...
function draw() {
  ...
  let num_clouds = round(random(1,20));
  let circle_rad = random(cloud_min_radius,cloud_max_radius);
  for(let i=0; i<num_clouds; i++){
    push();
    let cloud_width = random(10,40)*global_scale;
    let cloud_height = random(cloud_width, cloud_width*4);
    translate(random(cloud_width*2.5, canvas_x-cloud_width*2.5), random(cloud_height/2, canvas_y/3));
    cloud(cloud_width, cloud_height, i*50, false, circle_rad, multi_color);
    pop();
  }
...
//***************************************************
//custom funcs
function cloud(cloud_width, cloud_height, noise_offset, rain, circle_rad, random_background_color){
  ...
  const x_upper_limit = cloud_width*2*gaussian_std;
  const circle_data = [];
