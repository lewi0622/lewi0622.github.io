...
function draw() {
  ...
  for(let p=0; p<4; p++){
    ...
    push();
    if(p==0)translate(canvas_x/4, canvas_y/4);
    else if (p==1)translate(canvas_x*3/4, canvas_y/4);
    else if (p==2)translate(canvas_x/4, canvas_y*3/4);
    else translate(canvas_x*3/4, canvas_y*3/4);
    let rotation = 0;
