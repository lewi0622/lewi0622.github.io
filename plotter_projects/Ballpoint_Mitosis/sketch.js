...
function draw() {
  ...
  const shape_max_height = canvas_y/4;
  ...
  for(let j=0; j<2; j++){
    ...
    for(let i=0; i<num_trees; i++){
      ...
      line(0,0,0, -y-end_y);
      hatch(0, random(0.75,1.1) * (-y-end_y), hatch_steps); 
      pop();
    }
    pop();
  }
...
//***************************************************
//custom funcs
function hatch(end_x, end_y, steps){
...
x += x_step;
    ...
    squiggle(5, hatch_width/2, hatch_width, i);
    pop();
  }
}
...
function mask(){
  ...
  vertex(-canvas_x/2, -canvas_y/2);
  for(let i=0; i<steps; i++){
    const x = radius*cos(i * angle_step + 180);
    const y = -canvas_y/2 + radius*sin(i * angle_step +180);
    vertex(x,y);
  }
  endShape(CLOSE);
  pop();
