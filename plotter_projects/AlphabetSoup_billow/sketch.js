...
function gui_values(){
  ...
  parameterize("y_amp", 10, 0, 100, 0.1, true);
  parameterize("x_damp", 10, 1, 200, 1, false);
  parameterize("y_damp", 10, 1, 200, 1, false);
} 
...
function draw() {
  ...
  const points = font.textToPoints(random(["G"]), 0,0, font_size, { sampleFactor:  2 });
  ...
  for(let i=0; i<rows; i++){
    for(let j=0; j<cols; j++){
      ...
      if(tile == 1){
        ...
        const y_offset = map(noise(i/x_damp, j/y_damp), 0,1, -y_amp, y_amp);

        vertex(j * grid_size, i * grid_size + y_offset);
