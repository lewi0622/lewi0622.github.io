...
//TODO determine if it's possible to loop the colors
//determine if it's possible to move the lines as a loop
...
function draw() {
  ...
  const theta = angle_loop(fr, capture_time, 1);

  center_rotate(map(sin(theta), -1,1, -360,360));
  background("BLACK");

  let weight = 8 * global_scale;
  ...
  color_count++;
  drawingContext.shadowBlur=map(sin(theta * 10), -1,1, 2,5)*global_scale;
